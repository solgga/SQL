...
-- inner join 과 outer join 의 차이점을 설명하기 위해서 추가함.
...
--> left outer join 에서 outer 는 생략 가능
...
--> full outer join에서 outer는 생략가능함.

-- Oracle은 full outer join 문법을 제공하지 않음.
-- 집합 연산(합집합union, 교집합intersection , 차집합)을 사용.
...
-- join salgrade s on e.sal >= s.losal and e.sal <= s.hisal; 위랑 같음 같은조건임
...
from emp e
    join dept d on e.deptno = d.deptno
    join salgrade s on e.sal between s.losal and s.hisal
order by d.dname, s.grade; --표준방식

select e.ename, d.dname, e.sal, s.grade
from emp e, dept d, salgrade s
where e.deptno = d.deptno and e.sal between s.losal and s.hisal
order by d.dname, s.grade; --오라클방식
...
select e1.deptno, d.dname, e1.empno, e1.ename, e1.mgr, e2.ename as MGRNAME, e1.sal, s.grade
from emp e1
    left join emp e2 on e1.mgr = e2.empno
...
select d.deptno, d.dname, e1.empno, e1.ename, e1.mgr, e2.ename as MGRNAME, e1.sal, s.grade
from emp e1, emp e2, dept d, salgrade s
where e1.mgr = e2.empno and e1.deptno = d.deptno and e1.sal between s.losal and s.hisal and e1.sal >= 3000
order by e1.deptno, e1.empno;